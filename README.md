...
> バッテリ残量は電池電圧（GPIO10 の ADC 分圧）からの**推定値**です。負荷で電圧が沈むため数%〜数十%前後します。Cardputer ADV は充電信号線が無く「充電中」を直接検出できないため、**電圧が高い（USB給電/満充電付近, 既定 ≥4.10V）ときは % の代わりに水色の `CHG` を表示**します。`src/config.h` の `BAT_CHG_MV` でしきい値を調整できます。
...
「地域の設定を変える」とは実質、**その地域で聴ける局をプリセットに入れ替える**ことです。

### かんたん: 実機の `a` キーから選ぶ（推奨）

`a` キーで都道府県一覧（北海道〜沖縄）を開き、自分の地域を選ぶと、その地域の局リストを radiko から取得して本体（NVS）に保存します。次回起動からその局リストが使われます。ビルドや `stations.h` 編集は不要です。

> これは**セットアップ補助**の機能です。実際に聴けるのは接続元 IP のエリアの局だけなので、居住エリア（＝いま接続している回線のエリア）を選んでください。別エリアを選ぶと局は並びますが、再生時に「配信対象外」になります。

### 上級: ソースを書き換えて既定を変える

初期局や既定プリセットをコード側で固定したい場合の手順:
